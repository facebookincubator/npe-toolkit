 ...
 */
export type UseAction = () => Handler & {use: true};

/**
 * Utility to create a `UseAction`.
 */
export function actionHook(hook: () => Handler): UseAction {
  const result = hook as UseAction;
  /** @ts-ignore */
  result.use = true;
  return result;
}
 ...
 */
export type Action = Handler | UseAction;
...
function executeHooks(action: Action): Handler {
  if ('use' in action && action['use']) {
    return action() as Handler;
  } else {
    return action as Handler;
  }
}
...
export function useAction(action: Action, opts?: ActionOpts): ActionStatus;
...
export function useAction(
  arg0: string | Action,
  arg1?: Action | ActionOpts,
  arg2?: ActionOpts,
): ActionStatus {
  // See overloads above
  const action = typeof arg1 === 'function' ? arg1 : (arg0 as Action);
...
export function useActionImpl(
  ...
  async function wrappedHandler() {
    try {
      setPending(true);
      setError(undefined);
      await withTimeout(handler, timeout);
    } catch (e: any) {
      setError(e as Error);
    } finally {
      setPending(false);
